package cn.edu.ustc.analyzer.tpl;

import cn.edu.ustc.common.ActiveRecord;

public class JobInfo2 extends ActiveRecord {
	private String cpnm,jbnm,cpscale,cptype,cptrade,genderrq,rcnos,agerq,wkform,deadline,degreerq,salary,wkxp,wkplace,jbdesc,wkrq,benefit,date,cpinfo;

	public String getCpnm() {
		return cpnm;
	}

	public void setCpnm(String cpnm) {
		this.cpnm = cpnm;
	}

	public String getJbnm() {
		return jbnm;
	}

	public void setJbnm(String jbnm) {
		this.jbnm = jbnm;
	}

	public String getCpscale() {
		return cpscale;
	}

	public void setCpscale(String cpscale) {
		this.cpscale = cpscale;
	}

	public String getCptype() {
		return cptype;
	}

	public void setCptype(String cptype) {
		this.cptype = cptype;
	}

	public String getCptrade() {
		return cptrade;
	}

	public void setCptrade(String cptrade) {
		this.cptrade = cptrade;
	}

	public String getGenderrq() {
		return genderrq;
	}

	public void setGenderrq(String genderrq) {
		this.genderrq = genderrq;
	}

	public String getRcnos() {
		return rcnos;
	}

	public void setRcnos(String rcnos) {
		this.rcnos = rcnos;
	}

	public String getAgerq() {
		return agerq;
	}

	public void setAgerq(String agerq) {
		this.agerq = agerq;
	}

	public String getWkform() {
		return wkform;
	}

	public void setWkform(String wkform) {
		this.wkform = wkform;
	}

	public String getDeadline() {
		return deadline;
	}

	public void setDeadline(String deadline) {
		this.deadline = deadline;
	}

	public String getDegreerq() {
		return degreerq;
	}

	public void setDegreerq(String degreerq) {
		this.degreerq = degreerq;
	}

	public String getSalary() {
		return salary;
	}

	public void setSalary(String salary) {
		this.salary = salary;
	}

	public String getWkxp() {
		return wkxp;
	}

	public void setWkxp(String wkxp) {
		this.wkxp = wkxp;
	}

	public String getWkplace() {
		return wkplace;
	}

	public void setWkplace(String wkplace) {
		this.wkplace = wkplace;
	}

	public String getJbdesc() {
		return jbdesc;
	}

	public void setJbdesc(String jbdesc) {
		this.jbdesc = jbdesc;
	}

	public String getWkrq() {
		return wkrq;
	}

	public void setWkrq(String wkrq) {
		this.wkrq = wkrq;
	}

	public String getBenefit() {
		return benefit;
	}

	public void setBenefit(String benefit) {
		this.benefit = benefit;
	}

	public String getDate() {
		return date;
	}

	public void setDate(String date) {
		this.date = date;
	}

	public String getCpinfo() {
		return cpinfo;
	}

	public void setCpinfo(String cpinfo) {
		this.cpinfo = cpinfo;
	}

}
